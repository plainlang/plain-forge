...
- A single file inside the project, reachable by a path relative to where the renderer is run — parent-directory traversal (`../`) is allowed (`resources/` is the conventional home).
...
- [ ] Path resolves to a real text file on disk from where the renderer is run (`../` traversal is allowed)
